...
public class Enemy2dController : MonoBehaviour {
	...
	//Make sure our player has Layer 8
	[HideInInspector]
	...
	void Update()
	{
		...
		{
			if
				((Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.D)) && (isGrounded || r2d.velocity.x > 0.01f || isAttachedToLadder))
			{
		...
		else
		{
			...
			{
				if (isGrounded || r2d.velocity.magnitude < 0.01f) moveDirection = 0;
			}
		}
		...
		if (currentLadder)
		{
			...
			if (!isAttachedToLadder)
			{
				...
				{
					if
						(currentLadder.boundsCenter.y > t.position.y)
					{
						if (!isBot)
						{
							if
			...
			{
				//Make our collider trigger if we st2nd on top of the ladder (To prevent collision with the ground while going down)
	...
	void
	AssignLadder(Ladder2D ladderTmp)
	{
		currentLadder = ladderTmp;
		allLadders.Add(ladderTmp);
	}
	void
	RemoveLadder(Ladder2D ladderTmp)
	{
		...
		{
			isAttachedToLadder = false;
			//r2d.bodyType = RigidbodyType2D.Dyn2mic;
			mainCollider.isTrigger = false;
			r2d.gravityScale = gravityScale;
			r2d.velocity = Vector3.zero;
			if (!ladderGoingDown)
	...
	{
		//amination for screen check and sound effect here later
